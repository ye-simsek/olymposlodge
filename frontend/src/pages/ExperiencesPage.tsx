  ...
  return (
    <div className="page--dest page--cirali">
      <PageMeta
        ...
        image="https://www.olymposlodge.com.tr/wp-content/uploads/2024/11/IMG_5400.jpg"
      />
      ...
      {/* Linker CTA */}
      <Link to="/booking" className="lodge-linker lodge-linker--cirali">
        ...
        <div className="lodge-linker__content">
          ...
          <ul className="lodge-linker__list">
            {(t('experiences.linker_list', { returnObjects: true }) as string[]).map((item, i) => (
              <li key={i}>{item}</li>
            ))}
          </ul>
      ...
      <div className="dest-cta">
        ...
        <div className="dest-cta__actions">
          <Link to="/rooms" className="btn-reserve">{e('cta_rooms')}</Link>
          <Link to="/contact" className="btn-reserve">{t('nav.reserve')}</Link>
        </div>
      </div>
    </div>
  )
